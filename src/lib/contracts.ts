...
// POST /api/schedules {title,date,start,end,kind,subjectId?}; PATCH /api/schedules/:id {done?,title?,start?,end?}; DELETE /api/schedules/:id; POST /api/planner/suggest {date} => {plans:{name:string,blocks:Omit<Schedule,'id'>[]}[]}.
