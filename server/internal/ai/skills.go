...
var Skills = map[Kind]Skill{
	KindQuiz:    {ID: "memoryz.quiz", Version: "2.8.0", Instructions: "원문에 근거한 정확히 5지선다 문항을 작성한다. 정답 인덱스는 0~4이며 중복 선택지를 금지한다. 직접 인용으로 근거를 제시한다."},
	KindEssay:   {ID: "memoryz.essay", Version: "2.11.0", Instructions: "서술형 인출 문제에 원인부터 결과 순서의 정답 키워드 4개와 서로 겹치지 않는 방해 키워드 4개를 제공한다. 모범 답안과 원문 직접 인용을 포함한다. 정답과 방해어가 같은 판단 기준에 답하도록 가능·우세·유일·필요조건을 구분한다. 방해어를 틀렸다고 만들기 위해 문항에 없는 숨은 조건을 가정하지 않는다. 질문 조건 안에서 함께 참일 수 있는 해석이 있으면 그 방해어를 바꾸거나 원문으로 확인되는 조건을 질문에 명시한다. 구조·동일함 같은 말의 비교 대상과 기준을 명확히 한다."},
	KindCards:   {ID: "memoryz.cards", Version: "2.8.0", Instructions: "개념·관계·비교 중 학습 목표에 맞는 플래시카드를 만든다. 앞면과 뒷면은 하나의 인출 목표를 다루며 원문 인용을 포함한다."},
	KindGrade:   {ID: "memoryz.grade", Version: "2.9.3", Instructions: "동의어와 정확한 바꾸어 쓰기를 인정한다. 핵심 개념 60점, 인과 논리 25점, 설명 완결성 15점으로 평가한다. 키워드 단순 나열과 실제 의미 이해를 구분하고 올바른 부분 답안에도 점수를 인정한다. 중복된 키워드 표기는 별도의 요구사항이나 추가 감점으로 취급하지 않는다. 옳은 개별 결과의 인정과 답안 전체의 인과 완성도를 분리한다."},
