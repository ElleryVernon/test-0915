...
원장 본문은 `.unlazy/memoryz-cloud/gates/`(무시됨), 계획은 `.unlazy/memoryz-cloud/PLAN.md`. 아래 표는 2026-09-15 21:07 UTC 에 `unlazy-check --status` 로 잰 값이다.
...
| leaf-1.7 | leaf-1.7 캐싱 설계·Valkey(IAM+TLS)·관측성(OpenTelemetry) | 4 | 0 | ALL MET |
...
| leaf-3.2 | leaf-3.2 컨테이너·Cloud Run·마이그레이션 잡·시드·스윕·업타임 | 4 | 2 | open: J1, J4 |
| leaf-3.3 | leaf-3.3 Google OAuth 클라이언트·시크릿·클라우드 스모크(로그인) | 0 | 3 | open: O1, O2, O3 |
| leaf-4 | leaf-4 문서·커밋 | 0 | 3 | open: X1, X2, X3 |
| leaf-5 | leaf-5 모든 워크트리 → main 머지·로컬 적용 | 2 | 0 | ALL MET |
| leaf-6 | leaf-6 AI 기능 품질 검수 · 프로바이더 정합 | 7 | 3 | open: Q5, Q9, Q10 |
| leaf-7 | leaf-7 홈 "이어서 하기" 빈 상태 · 복습 CTA | 5 | 0 | ALL MET |
| leaf-8 | leaf-8 사용자 도메인 memoryz.kr 연결 | 4 | 2 | open: D4, D6 |
| leaf-9 | leaf-9 지터(jitter) 시스템 설계 검토·개선 | 10 | 0 | ALL MET |

합계: 충족 101 · 미충족 15 · 포기 0.
<!-- /memoryz-cloud index -->
