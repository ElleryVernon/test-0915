...
test('month grid: 42 cells starting on the Sunday before the 1st, with today, selection and range', () => {
  ...
  assert.deepEqual(
    grid.slice(0, 3).map((c) => [c.date, c.inMonth]),
    [
      ['2026-08-30', false],
      ['2026-08-31', false],
      ['2026-09-01', true],
...
test('inline validation messages replace the browser bubble', () => {
  ...
  assert.equal(
    validationMessage('ab', { label: '닉네임', minLength: 3 }),
    '닉네임을 3자 이상 입력해 주세요.',
  );
  assert.equal(
    validationMessage('a'.repeat(21), { label: '시험 이름', maxLength: 20 }),
    '시험 이름을 20자 이내로 줄여 주세요.',
  );
  assert.equal(
    validationMessage('abc', {
      label: '코드',
      pattern: /^\d{6}$/,
      patternMessage: '숫자 6자리예요.',
    }),
    '숫자 6자리예요.',
  );
...
test('Monday-first planner picker preserves dates and weekday identities', () => {
  const grid = monthGrid(2026, 9, { weekStartsOn: 1, selected: '2026-09-17' });
  assert.equal(grid[0].date, '2026-08-31');
  assert.equal(grid[0].weekday, 1);
  assert.equal(grid.find((cell) => cell.selected)?.date, '2026-09-17');
  assert.equal(grid.find((cell) => cell.selected)?.weekday, 4);
});
